...
// Data URI Fallback Image for Placeholder 
const FALLBACK_LOGO_URL = "https://placehold.co/102x35/ff0000/ffffff?text=LOGO+SPBU"; 
...
// --- PERUBAHAN KRUSIAL: Footer baru dengan 5 baris underscore ---
...
const initialSpbuConfig = {
    ...
    receiptWidth: 450, // Lebar nota default
    id: generateId(),
    logoBase64: null, 
};
...
// Utility for formatting the receipt transaction row (for stable alignment)
const ReceiptRow = React.memo(({ label, value, isTotal = false }) => {
    // Tentukan kelas CSS berdasarkan apakah ini baris total atau bukan
    const labelClasses = `font-normal w-32 flex-shrink-0 whitespace-nowrap`;
    
    let valueClasses = 'text-base'; // Set default size eksplisit
    let containerClasses = 'text-base';

    if (isTotal) {
        // Menggunakan font-extrabold dan menjaga ukuran teks di text-base
        valueClasses = 'text-base font-extrabold'; 
        // Mengubah label total menjadi lebih tebal (font-bold)
        containerClasses = 'text-base font-bold'; 
    } else {
        // Hanya memastikan size default (font-normal untuk nilai non-total)
        valueClasses = 'text-base font-normal';
    }

    return (
        // Menggunakan text-base untuk container, yang mengatur ukuran font label
        <div className={`flex justify-between ${containerClasses}`}> 
            <span className={labelClasses}>{label} :</span> 
            {/* Terapkan kelas spesifik ke nilai */}
            <span className={`flex-grow text-right ${valueClasses}`}>{value}</span>
        </div>
    );
});


// Utility for resizing image and getting Base64
...
const ReceiptView = React.forwardRef(({ spbu, transaction, finalPrice, calculatedVolume }, ref) => {
    // PERUBAHAN KRUSIAL 1: Mengubah font dari Courier ke Arial/Sans
    const receiptStyle = "font-sans text-black leading-none bg-white p-2";
        ...
        <div ref={ref} className={receiptStyle} style={{ width: `${spbu.receiptWidth || 450}px`, margin: '0 auto' }}>
            ...
            <div className="flex justify-center mb-1">
                <img 
                    src={logoSrc} 
                    alt="Logo SPBU" 
                    // Perubahan: Mengatur lebar menjadi 102px (setengah dari 205px)
                    className="w-[102px] h-auto max-w-full filter grayscale" 
                    onError={(e) => { 
            ...
            {/* Mengubah text-xl menjadi text-lg, text-lg menjadi text-base */}
            <div className="text-center font-bold mb-1"> 
                <p className="text-lg">{spbu.name}</p>
                {addressLines.map((line, i) => <p key={i} className="text-base font-normal">{line}</p>)}
            </div>

            <div className="border-t border-b border-dashed border-black my-1 h-0"></div>
            
            {/* START: BLOK DATA TANGGAL/SHIFT/NOTA */}
            {/* Mengubah text-lg menjadi text-base */}
            <div className="grid grid-cols-2 gap-x-2 text-base text-center">
                {/* Kolom Kiri: Shift dan Tanggal */}
                <div>
                    <p className="font-medium text-left">Shift:</p>
                    <p className="text-left">{transaction.shift}</p>
                </div>
                {/* Kolom Kanan: No. Transaksi (DIUBAH DARI No. Nota) */}
                <div>
                    <p className="font-medium text-right">No.Trans:</p>
                    <p className="text-right">{transaction.noTrans}</p>
                </div>
                
                {/* Pindah baris */}
                <div className="col-span-2 border-t border-dashed border-black my-1 h-0"></div>

                {/* Kolom Kiri Bawah: Tanggal */}
                <div>
                    <p className="font-bold text-left">Tanggal:</p>
                    <p className="font-bold text-left">{transaction.date}</p>
                </div>

                {/* Kolom Kanan Bawah: Jam */}
                <div>
                    <p className="font-bold text-right">Jam:</p>
                    <p className="font-bold text-right">{transaction.time}</p>
                </div>
            </div>
            ...
            {/* Blok Data Transaksi Utama: space-y-0 */}
            {/* Mengubah text-lg menjadi text-base */}
            <div className="space-y-0 text-base"> 
                <ReceiptRow label="Pulau/Pompa" value={transaction.islandPump} />
                <ReceiptRow label="Nama Produk" value={transaction.productName} />
                <ReceiptRow label="Harga/Liter" value={`Rp. ${formatRupiah(priceLiter)}`} />
                {/* Tampilkan Volume dengan 2 angka desimal */}
                <ReceiptRow label="Volume" value={` (L) ${displayVolume.toFixed(2)}`} />
                
                {/* Total Harga - Menggunakan ReceiptRow yang sudah dimodifikasi */}
                <ReceiptRow label="Total Harga" value={`Rp. ${formatRupiah(displayPrice)}`} isTotal={true} />

                <ReceiptRow label="Operator" value={transaction.operator} />
                <ReceiptRow label="Nopol" value={transaction.nopol} />
            </div>

            <div className="border-t border-b border-dashed border-black my-1 h-0"></div>
            
            {/* Footer diubah dari text-xs (12px) menjadi text-base (16px) */}
            <div className="text-center mt-2 text-base whitespace-pre-line leading-tight font-bold">
                {/* Menampilkan baris footer (sekarang termasuk " ###### ") */}
                {footerLines.map((line, i) => <p key={i}>{line || ' '}</p>)} 
                {/* ' ' untuk menjaga baris kosong tetap dirender */}
            </div>
        </div>
    );
});

// Utility untuk resize gambar dan mendapatkan Base64 (dibiarkan karena tidak terpengaruh error)
// ...
...
export default function App() {
    ...
    // Max width untuk resize logo 
    const LOGO_MAX_WIDTH = 102;
    ...
    useEffect(() => {
        ...
        // Jika SPBU dipilih, gunakan datanya, tetapi timpa footer dengan teks terkunci
        if (selected) setCurrentSpbu({...selected, footerNote: LOCKED_FOOTER_TEXT}); 
        else if (spbuList.length > 0) { 
            setSelectedSpbuId(spbuList[0].id); 
            setCurrentSpbu({...spbuList[0], footerNote: LOCKED_FOOTER_TEXT}); 
        }
    ...
    const handleSpbuChange = (e) => {
        // Abaikan perubahan jika nama input adalah 'footerNote' karena sudah dikunci
        if (e.target.name === 'footerNote') return; 

        setCurrentSpbu(prev => ({ 
            ...prev, 
            [e.target.name]: e.target.name === 'receiptWidth' ? (parseInt(e.target.value) || 450) : e.target.value 
        }));
    }
    ...
    const handleImageUpload = (e) => {
        ...
        // Resize gambar ke lebar maksimal 102px (LOGO_MAX_WIDTH) sebelum Base64
    ...
    const handleSaveSpbu = async () => {
        ...
        try {
            // *** MENGUNCI FOOTER TEXT SAAT MENYIMPAN ***
            const spbuToSave = { 
                ...currentSpbu, 
                // Pastikan footerNote selalu menggunakan teks yang terkunci sebelum disimpan
                footerNote: LOCKED_FOOTER_TEXT, 
                // Bersihkan ID jika ini adalah template baru
                id: currentSpbu.id || generateId()
            }; 
            
            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'spbu_configs', spbuToSave.id), spbuToSave, { merge: true });
            
            // Perbarui currentSpbu dengan data yang telah disimpan (termasuk footerNote yang terkunci)
            setCurrentSpbu(spbuToSave); 
    ...
    const handleAddNewSpbu = () => {
        // Pastikan footerNote tetap terkunci untuk template baru
        setCurrentSpbu({ ...initialSpbuConfig, id: generateId(), name: 'SPBU BARU', logoBase64: null, footerNote: LOCKED_FOOTER_TEXT });
    ...
    const generateCanvas = async () => {
        ...
        // --- Perubahan Kunci di Sini: Menghitung Skala untuk Konsistensi ---
        const desiredScale = 3; // Skala upscaling yang diinginkan (contoh: 450px * 3 = 1350px resolusi akhir)
        const devicePixelRatio = window.devicePixelRatio || 1;
        const finalScale = desiredScale / devicePixelRatio; // Mengimbangi kerapatan piksel perangkat
        
        const width = currentSpbu.receiptWidth || 450;
        // -------------------------------------------------------------------

        return await window.html2canvas(receiptRef.current, { 
            scale: finalScale, // Menggunakan skala yang sudah dinormalisasi
            useCORS: true, 
    ...
    const handleAiGenerateTemplate = async () => {
        ...
        try {
            ...
            setCurrentSpbu(prev => ({ 
                ...
                // Abaikan footerNote dari AI dan tetap gunakan yang terkunci
            ...
            <div className="w-full max-w-md">
                ...
                {isEditingSpbu ? (
                    <div className="mb-6 p-4 bg-white rounded-lg shadow-xl border-t-4 border-blue-500 space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className='flex justify-between items-center'>
                                <p className="text-xs text-red-500">
                                    PENTING: Gambar akan diresize otomatis (maks {LOGO_MAX_WIDTH}px) dan dikonversi ke Base64 agar dapat disimpan.
                                </p>
                        ...
                        {/* INPUT FOOTER TERKUNCI */}
                        <div>
                            <label className="text-sm block">Footer (Terkunci)</label>
                            <textarea 
                                name="footerNote" 
                                value={LOCKED_FOOTER_TEXT} 
                                rows={6} // Ubah rows agar cukup untuk 6 baris
                                disabled 
                    ...
                    <>
                        <div className="mb-6 p-4 bg-gray-50 rounded-lg shadow-inner">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <Input 
                                    label="Tanggal" // DIUBAH: Tgl -> Tanggal
                                    name="date" 
